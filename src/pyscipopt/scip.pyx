...
cdef class Model:
    ...
    def getKhalilState(self, root_info, candidates):
        ...
        for cand_i in range(ncands):
            ...
            cand_cdeg_mean_ratio[cand_i] = 0 if cdeg_mean == 0 else cdeg_mean / (
                        cand_root_cdeg_mean[cand_i] + cdeg_mean)
    ...
    def getSolvingStats(self):
        ...
        cdef np.ndarray[np.float_t, ndim=1] lowerbounds = np.empty([nleaves + nchildren + nsiblings + 1],
                                                                   dtype=np.float)
        ...
        return {
            ...
            'repropdepth': SCIPgetRepropdepth(scip),
            ...
            'ntotalnodes': SCIPgetNTotalNodes(scip),
            # total number of processed nodes in all runs, including the focus node
            'nfeasibleleaves': SCIPgetNFeasibleLeaves(scip),
            ...
            'ndelayedcutoffs': SCIPgetNDelayedCutoffs(scip),
            ...
            'nbacktracks': SCIPgetNBacktracks(scip),
            ...
            'lowerboundroot': SCIPgetLowerboundRoot(scip),
            ...
            'firstlplowerboundroot': SCIPgetFirstLPLowerboundRoot(scip),
